...
namespace Mission11.Controllers
{
    ...
    public class BooksController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<Book>>> GetBooks(int page = 1, int pageSize = 5, string sortBy = "Title", string sortOrder = "asc", string category = null)
        {
            ...
            // Log the classification values of the fetched books for debugging
            foreach (var book in books)
            {
                Console.WriteLine($"Fetched Book - Title: {book.Title}, Classification: {book.Classification}");
            }
        ...
        {
            // Ensure Classification is not null or empty
            if (string.IsNullOrWhiteSpace(book.Classification))
            {
                book.Classification = "Unclassified";  // Provide a default value
            }

            // Log for debugging purposes
            Console.WriteLine($"Book Classification: {book.Classification}");
