...
import ManageBooks from './ManageBooks'; // Import for navigating to the Manage Books page
...
const BooksList = () => {
    ...
    useEffect(() => {
        const fetchBooks = async () => {
            try {
                ...
                // Add console.log to check the structure of books
                console.log(response.data.books);  // Log the response to inspect the structure
                setBooks(response.data.books);  // Set books as an array
                setTotalPages(response.data.totalPages);  // Set total pages for pagination
            } catch (error) {
                console.error("Error fetching books:", error);
                setBooks([]);  // Ensure books is always an array in case of error
            }
        };
    ...
    // Navigate to the Admin page to manage books (You may need to create this page)
    const handleManageBooks = () => {
        navigate('/admin/manage-books');  // Update with the correct path to manage books
    };
    ...
    return (
        <Container className="my-4">
            ...
            {/* Manage Books Button (for Admins) */}
            <Button
                onClick={handleManageBooks}
                className="mb-4"
                variant="danger"  // Style it as an admin button
            >
                Manage Books
            </Button>
            ...
            <Row className="mb-4">
                {Array.isArray(books) && books.length > 0 ? (
                    ...
                    <p>No books available.</p>  // Display this if books is not an array or is empty
                )}
            </Row>
